...
def send_aggregated_notification(notifier: TelegramNotifier, new_listings: list, total_scraped: int, db_count: int):
    """Send a scan summary notification. Always sent, whether or not new listings were found."""

    now_str = datetime.now().strftime('%d/%m/%Y %H:%M')

    if not new_listings:
        message = (
            f"🔍 *Daily Rental Scan — {now_str}*\n"
            f"━━━━━━━━━━━━━━━━━━━━━\n"
            f"✅ Scan complete — no new listings found.\n"
            f"📊 Checked: {total_scraped} listings | DB total: {db_count}"
        )
    ...
    message += f"\n📊 Checked: {total_scraped} listings | DB total: {db_count}"
...
def run_scraping_job():
    ...
    try:
        ...
        # Send aggregated notification (always sent, even if no new listings)
        send_aggregated_notification(notifier, new_listings, total_scraped, db.get_listing_count())
...
def main():
    ...
    # Add job for 14:00 Jerusalem time
    scheduler.add_job(
        run_scraping_job,
