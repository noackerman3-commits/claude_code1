...
def send_aggregated_notification(notifier: TelegramNotifier, new_listings: list, total_scraped: int):
    """Send a daily scan summary. Always fires so you know the scan ran."""
    ...
    if not new_listings:
        message = (
            ...
            f"📊 Total checked: {total_scraped}"
        )
        notifier.send_message(message)
        logger.info("No new listings — sent scan-complete notification")
        return
