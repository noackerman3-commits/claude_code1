...
Runs at 14:00 (2:00 PM) Israel time daily.
"""
import sys
import os
...
def send_aggregated_notification(notifier: TelegramNotifier, new_listings: list, total_scraped: int = 0):
    """Send a single notification with scan results (always sent, even if no new listings)."""
    ...
    if not new_listings:
        ...
        message += f"✅ No new listings found\n"
        message += f"_Next scan: tomorrow at 14:00_"
        notifier.send_message(message)
        logger.info("Sent scan summary — no new listings")
        return
...
def run_scraping_job():
    ...
    try:
        ...
        # Send aggregated notification (always sent so user knows the scan ran)
