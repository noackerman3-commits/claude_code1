...
def send_aggregated_notification(notifier: TelegramNotifier, new_listings: list, total_scraped: int, total_in_db: int):
    """Send a scan-result summary notification, always — even when no new listings."""

    scan_time = datetime.now().strftime('%d/%m/%Y %H:%M')

    if not new_listings:
        message = (
            f"🔍 *Daily Rental Scan — {scan_time}*\n"
            f"━━━━━━━━━━━━━━━━━━━━━\n"
    ...
    if len(new_listings) > 10:
        message += f"_+ {len(new_listings) - 10} more listings in database_\n\n"

    message += f"📊 Scanned: {total_scraped}  |  Total saved: {total_in_db}"
...
def run_scraping_job():
    ...
    try:
        ...
        # Send aggregated notification (always — even when 0 new listings)
        total_in_db = db.get_listing_count()
        send_aggregated_notification(notifier, new_listings, total_scraped, total_in_db)
