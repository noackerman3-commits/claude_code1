...
Runs once daily at 14:00 Jerusalem time.
"""
import sys
import os
...
def send_aggregated_notification(notifier: TelegramNotifier, new_listings: list, total_scraped: int = 0):
    """Send a daily scan summary — always, even when nothing new is found."""
    ...
    if not new_listings:
        message = (
            ...
            f"_Checked {total_scraped} listing(s) in total._"
        )
...
def run_scraping_job():
    ...
    try:
        ...
        # Send aggregated notification (always sends — even if nothing new)
