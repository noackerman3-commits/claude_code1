...
Runs at 14:00 Israel time daily.
"""
import sys
import os
...
def send_aggregated_notification(notifier: TelegramNotifier, new_listings: list, total_scraped: int = 0):
    """Send a single notification with all new listings, or a status message if none."""
    ...
    if not new_listings:
        ...
        message += f"⏰ {scan_time}\n"
        message += f"📊 Checked {total_scraped} listings — no new matches today."
        notifier.send_message(message)
        logger.info("Sent scan status: no new listings")
        return
...
def main():
    ...
    # Add single daily job at 14:00 Jerusalem time
    scheduler.add_job(
        run_scraping_job,
